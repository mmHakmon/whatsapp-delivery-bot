...
if (o.sender_phone) {
  ...
  const courierVehicle = cr.rows[0].vehicle_type ? vehicleTypeHe[cr.rows[0].vehicle_type] : '🚗 רכב';
  
  const customerMessage = `🚚 *המשלוח שלך נתפס!*

📦 משלוח מספר: #${o.order_number}

👤 *פרטי השליח:*
שם: ${cd.firstName} ${cd.lastName}
📞 טלפון: ${cd.phone}
${courierVehicle}
...
השליח כבר בדרך אליך! 🏍️💨

במקרה של שאלות, פנה לשליח בטלפון: ${cd.phone}`;

  try {
    await sendWhatsApp(o.sender_phone, customerMessage);
    console.log('✅ Customer notification sent to:', o.sender_phone);
  } catch (error) {
